...
Arguments:
    ${DISKGUARD_COLOR_CYAN} -s|s|--status|status${DISKGUARD_COLOR_NC}      → show plugin status
...
body_lines+=("
    ...
    ${DISKGUARD_COLOR_CYAN} -st|--scan-threshold 500${DISKGUARD_COLOR_NC}  → set ${DISKGUARD_COLOR_BLUE_DIM}DISKGUARD_SCAN_THRESHOLD=500${DISKGUARD_COLOR_NC}
                                 ${DISKGUARD_COLOR_GREY}and reload automatically${DISKGUARD_COLOR_NC}
...
This will now be the default setting until you change it
or delete the configuration file.")
            ...
            -st|--scan-threshold|scan-threshold)
                ...
                local title body
                title=("${DISKGUARD_COLOR_YELLOW_BOLD}DiskGuard${DISKGUARD_COLOR_NC}")
                footer=("Scan threshold updated to ${DISKGUARD_COLOR_CYAN}${DISKGUARD_SCAN_THRESHOLD}MB${DISKGUARD_COLOR_NC}
This will now be the default setting until you change
it or delete the configuration file.")
                diskguard_print_box \
                "${title[@]}" \
                "${footer}"
                unset val input
